...
import { Users, Loader2, Download, Search, ChevronLeft, ChevronRight, ArrowUpDown, FileText, FileSpreadsheet, ChevronDown } from 'lucide-react';
...
export default function VisitorsPage() {
  ...
  const fetchVisitors = async () => {
    ...
    try {
      ...
      const res = await apiClient.get(`/api/visitors/?${params.toString()}`);
  ...
  const handleCheckout = async (visitorId: number) => {
    ...
    try {
      ...
      setVisitors(prev => prev.map(v => 
        v.id === visitorId ? { ...v, checked_out_at: new Date().toISOString() } : v
      ));
    } catch (err: any) {
  ...
  const exportPDF = () => {
    if (visitors.length === 0) return;
    const doc = new jsPDF();
    doc.text(`Visitor Logs - ${new Date().toLocaleDateString()}`, 14, 15);
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 mt-6 md:mt-8">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors duration-300">
          ...
          <div className="px-4 md:px-6 py-4 md:py-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-colors">
            <div>
              <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg">Visitor Audit Logs</h3>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-500 dark:text-slate-400 space-y-4">
              ...
              <div>
                <h4 className="text-slate-800 dark:text-slate-200 font-bold">No Visitor Logs Found</h4>
                ...
                {visitors.map((log) => (
                  <div key={log.id} className="p-4 space-y-3 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950 rounded-lg p-3 border border-slate-100 dark:border-slate-800">
                      <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Visitor Details</p>
                    ...
                    <div className="flex items-center justify-between text-xs pt-2">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-blue-600 dark:text-blue-400">{log.campus_name}</span>
