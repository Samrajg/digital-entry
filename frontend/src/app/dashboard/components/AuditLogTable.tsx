...
import { Users, Truck, Loader2, Download, ChevronLeft, ChevronRight, ArrowUpDown, FileText, FileSpreadsheet, ChevronDown } from 'lucide-react';
...
export function AuditLogTable({ type, currentUser }: AuditLogTableProps) {
  ...
  }, [currentUser, skip, startDate, endDate, campusId, securityId, sortBy, sortOrder, type]);
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const endpoint = type === 'visitor' ? '/api/visitors/' : '/api/vehicles/';
  ...
  const handleCheckout = async (recordId: number) => {
    ...
    try {
      const endpoint = type === 'visitor' ? `/api/visitors/${recordId}/checkout` : `/api/vehicles/${recordId}/checkout`;
      await apiClient.post(endpoint);
      setRecords(prev => prev.map(v => 
        v.id === recordId ? { ...v, checked_out_at: new Date().toISOString() } : v
      ));
    } catch (err: any) {
      setError(err.response?.data?.detail || `Failed to check out ${type}`);
    } finally {
  ...
  const exportCSV = () => {
    ...
    const rows = records.map(v => {
      ...
      let baseRow = [v.id, dateStr, v.campus_name, v.gate_name, v.security_name].map(escapeCSV);
      const formDetails = Object.entries(v.form_data).map(([k, val]) => `${k}: ${val}`).join(' | ');
      baseRow.push(escapeCSV(formDetails));
      return baseRow.join(',');
    });
    ...
    link.setAttribute("download", `${type}_logs_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const exportPDF = () => {
    if (records.length === 0) return;
    const doc = new jsPDF();
    doc.text(`${type === 'visitor' ? 'Visitor' : 'Vehicle'} Logs - ${new Date().toLocaleDateString()}`, 14, 15);
    
    const tableColumn = ["Date/Time", "Campus", "Gate", "Authorized By", "Details"];
    const tableRows = records.map(v => {
    ...
    doc.save(`${type}_logs_${new Date().toISOString().split('T')[0]}.pdf`);
  };

  const totalPages = Math.ceil(total / limit);
  const currentPage = Math.floor(skip / limit) + 1;

  const Icon = type === 'visitor' ? Users : Truck;
  const title = type === 'visitor' ? 'Visitor Audit Logs' : 'Vehicle Audit Logs';
  const entityName = type === 'visitor' ? 'Visitor' : 'Vehicle';
  const colorClass = type === 'visitor' ? 'blue' : 'indigo';
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors duration-300">
      ...
      {loading ? (
        <div className="flex justify-center items-center py-20">
          <Loader2 className={`w-8 h-8 text-${colorClass}-600 dark:text-${colorClass}-400 animate-spin`} />
        </div>
      ) : error ? (
        ...
        <div className="text-center py-20 text-slate-500 dark:text-slate-400 space-y-4">
          ...
          <div>
            <h4 className="text-slate-800 dark:text-slate-200 font-bold">No {entityName} Logs Found</h4>
                      ...
                      {log.checked_out_at ? (
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <button
                            ...
                            className={`text-xs px-3 py-1.5 bg-${colorClass}-600 hover:bg-${colorClass}-700 text-white rounded-lg transition-colors disabled:opacity-50`}
                          >
            ...
            {records.map((log) => (
              <div key={log.id} className="p-4 space-y-3 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                ...
                <div className="bg-slate-50 dark:bg-slate-950 rounded-lg p-3 border border-slate-100 dark:border-slate-800">
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">{entityName} Details</p>
                ...
                <div className="flex items-center justify-between text-xs pt-2">
                  <div className="flex items-center gap-2">
                    <span className={`font-semibold text-${colorClass}-600 dark:text-${colorClass}-400`}>{log.campus_name}</span>
